...
public class ReportFixirovannieCeny extends Report_Base{
	...
	public String composeGetQuery(int queryKind){
		...
		if(kind.value().intValue() == 2){
			p = p + ",\"СтатусЗаявки\":\"Утверждена\"";
		}else{
			if(kind.value().intValue() == 3){
				p = p + ",\"СтатусЗаявки\":\"Отказ\"";
			}else{
				if(kind.value().intValue() == 1){
					p = p + ",\"СтатусЗаявки\":\"Не просмотрена\"";
				}
			}
		}
	...
	public SubLayoutless getParametersView(Context context){
		...
		if(propertiesForm == null){
			...
			propertiesForm//
					...
					.input(context, 6, Auxiliary.tapSize * 0.3, "Тип", new RedactSingleChoice(context).selection.is(kind)
							.item("Все")
							.item("Не просмотрена")
							.item("Утверждена")
							.item("Отказ")
			...
			propertiesForm.child(new Knob(context)//
					...
					.left().is(propertiesForm.shiftX.property.plus(Auxiliary.tapSize * (0.3 + 0 * 2.5)))//
					.top().is(propertiesForm.shiftY.property.plus(Auxiliary.tapSize * (1.5 * 7 + 3 + 0.5)))//
					.width().is(Auxiliary.tapSize * 2.5)//
					.height().is(Auxiliary.tapSize * 0.8)//
			);
		}
