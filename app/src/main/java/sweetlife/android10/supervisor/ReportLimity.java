...
public class ReportLimity extends Report_Base {
	...
	public ReportLimity(ActivityWebServicesReports p) {
		super(p);
	}
	@Override
	...
	propertiesForm.child(new Knob(context)//
			...
			.afterTap.is(new Task() {
				...
				public void doTask() {
					//expectRequery.start(activityReports);
					activityReports.promptDeleteRepoort(ReportLimity.this, currentKey);
				}
			})//
					.left().is(propertiesForm.shiftX.property.plus(Auxiliary.tapSize * (0.3 + 0 * 2.5)))//
