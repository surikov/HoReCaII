...
public class ReportDistribucia2 extends Report_Base {
	...
	public ReportDistribucia2(ActivityWebServicesReports p) {
		super(p);
	}
	...
	public String composeGetQuery(int queryKind) {
		...
		String p = "{\"ДатаНачала\":\"" + Cfg.formatMills(dateFrom.value(), "yyyyMMdd") + "\""//
				...
				+ ",\"КлючВарианта\":\"f230cba9-9bad-48ff-afa3-b23a6fdcd8ad\""//
				;
		p = p + "}";
		String e = "";
		try {
		...
		if (propertiesForm == null) {
			...
			propertiesForm.child(new Knob(context)//
					...
					.afterTap.is(new Task() {
						...
						public void doTask() {
							//expectRequery.start(activityReports);
							activityReports.promptDeleteRepoort(ReportDistribucia2.this, currentKey);
						}
					})//
					.left().is(propertiesForm.shiftX.property.plus(Auxiliary.tapSize * (0.3 + 0 * 2.5)))//
