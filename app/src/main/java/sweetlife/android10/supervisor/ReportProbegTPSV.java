...
public class ReportProbegTPSV  extends Report_Base {
	...
	public ReportProbegTPSV(ActivityWebServicesReports p) {
		super(p);
	}
	@Override
		...
		if (propertiesForm == null) {
			...
			propertiesForm.child(new Knob(context)//
					...
					.afterTap.is(new Task() {
						...
						public void doTask() {
							//expectRequery.start(activityReports);
							activityReports.promptDeleteRepoort(ReportProbegTPSV.this, currentKey);
						}
					})//
							.left().is(propertiesForm.shiftX.property.plus(Auxiliary.tapSize * (0.3 + 0 * 2.5)))//
