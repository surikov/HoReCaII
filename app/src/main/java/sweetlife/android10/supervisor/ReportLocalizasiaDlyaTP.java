...
public class ReportLocalizasiaDlyaTP extends Report_Base {
	Numeric dateCreateFrom = new Numeric();
	//Numeric dateCreateTo = new Numeric();
	Numeric territory = new Numeric();

	public static String menuLabel() {
		return "Локализация для ТП";
	}
	public static String folderKey() {
		return "localizasiaDlyaTP";
	}
	...
	public ReportLocalizasiaDlyaTP(ActivityWebServicesReports p) {
		super(p);
	}
	@Override
	...
	public SubLayoutless getParametersView(Context context) {
		if (propertiesForm == null) {
			...
			propertiesForm//
					...
					.input(context, 1, Auxiliary.tapSize * 0.3, "День", new RedactDate(context).date.is(dateCreateFrom).format.is("dd.MM.yyyy"))//
			...
			propertiesForm.child(new Knob(context)//
					...
					.afterTap.is(new Task() {
						...
						public void doTask() {
							//expectRequery.start(activityReports);
							activityReports.promptDeleteRepoort(ReportLocalizasiaDlyaTP.this, currentKey);
						}
					})//
							.left().is(propertiesForm.shiftX.property.plus(Auxiliary.tapSize * (0.3 + 0 * 2.5)))//
