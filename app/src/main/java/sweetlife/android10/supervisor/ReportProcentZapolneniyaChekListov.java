...
public class ReportProcentZapolneniyaChekListov extends Report_Base {
	...
	public ReportProcentZapolneniyaChekListov(ActivityWebServicesReports p) {
		super(p);
	}

	public static String menuLabel() {//list
		return "Процент заполнения чек-листов";
	}

	public static String folderKey() {
		return "procentZapolneniaCheckListov";
	}
	...
	public void writeDefaultForm(String instanceKey) {
		...
		b.child("shipFrom").value.is("" + (d + 0 * 24 * 60 * 60 * 1000.0));
		b.child("shipTo").value.is("" + (d + 0 * 24 * 60 * 60 * 1000.0));
...
*/
		String p = "{"//
				           + "\"ДатаНачала\":\"" + Cfg.formatMills(dateShipFrom.value(), "yyyyMMdd") + "\""//
				           + ",\"ДатаОкончания\":\"" + Cfg.formatMills(dateShipTo.value(), "yyyyMMdd") + "\""//
				           + "}";
		String e = "";
		try {
		...
		String serviceName = "ПроцентЗаполненияЧекЛистов";
		try {
			serviceName = URLEncoder.encode(serviceName, "UTF-8");
			//serviceName = "%CF%F0%EE%F6%E5%ED%F2%C7%E0%EF%EE%EB%ED%E5%ED%E8%FF%D7%E5%EA%CB%E8%F1%F2%EE%E2";
		} catch(Throwable t) {
			t.printStackTrace();
	...
	public SubLayoutless getParametersView(Context context) {
		...
		*/
		propertiesForm//
				...
				.input(context, 1, Auxiliary.tapSize * 0.3, "От", new RedactDate(context).date.is(dateShipFrom).format.is("dd.MM.yyyy"))//
		...
		*/
		...
		propertiesForm.child(new Knob(context)//
					...
					public void doTask() {
						...
						dateShipFrom.value(d - 0 * 24 * 60 * 60 * 1000.0);
						dateShipTo.value(d + 0 * 24 * 60 * 60 * 1000.0);
						expectRequery.start(activityReports);
					}
				})//
		...
		propertiesForm.child(new Knob(context)//
					...
					public void doTask() {
						//expectRequery.start(activityReports);
						activityReports.promptDeleteRepoort(ReportProcentZapolneniyaChekListov.this, currentKey);
					}
				})//
				                     .left().is(propertiesForm.shiftX.property.plus(Auxiliary.tapSize * (0.3 + 0 * 2.5)))//
