...
public class ReportStatistikaZakazov2 extends Report_Base{
	...
	//Vector<Note>artikuls=new Vector<Note>();
	...
	public String composeGetQuery(int queryKind){
		...
		if(artikul.value().trim().length() > 1){
			String artarr = artikul.value().trim();
			artarr = "\"" + artarr.replace(",", "\",\"") + "\"";
			artarr = artarr.replace("\" ", "\"");
			artarr = artarr.replace(" \"", "\"");
			artarr = artarr.replace("\"", "\"\"");
			artarr = artarr.replace("\"", "\"\"");
	...
	Vector<String> artsList(){
		Vector<String> arts = new Vector<String>();
		String[] aa = artikul.value().trim().split(",");
		for(int ii = 0; ii < aa.length; ii++){
			if(aa[ii].trim().length() > 1){
				arts.add(aa[ii].trim());
			}
		}
		return arts;
	...
	public SubLayoutless getParametersView(Context context){
		...
		propertiesForm//
				...
				.input(context, 7, Auxiliary.tapSize * 0.3, "Артикулы", new RedactText(context).text.is(artikul))//
		;
		Task tap = new Task(){
			public void doTask(){
				//arts.removeAllElements();
				//arts.add(artikul.value());
				Vector<String> arts = artsList();
				dialog.showArtDialog(context, arts, new Task(){
					public void doTask(){
						String artline = "";
						String dlmtr = "";
						for(int ii = 0; ii < arts.size(); ii++){
							if(arts.get(ii).trim().length() > 1){
								System.out.println("selection " + arts.get(ii));
								artline = artline + dlmtr + arts.get(ii).trim();
								dlmtr = ", ";
							}
						}
						System.out.println("artline " + artline);
						artikul.value(artline);
					}
				});
			}
		};
		propertiesForm.child(new Knob(context).labelText.is("...").afterTap.is(tap)
				.left().is(propertiesForm.shiftX.property.plus(5.2 * Auxiliary.tapSize))//
				.top().is(propertiesForm.shiftY.property.plus(10.8 * Auxiliary.tapSize))//
