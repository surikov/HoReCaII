...
public class ReportResultatyUtverjdenihSpecifikaciy extends Report_Base {
	...
	public ReportResultatyUtverjdenihSpecifikaciy(ActivityWebServicesReports p) {
		super(p);
	}

	public static String menuLabel() {
		return "Результаты утвержденных спецификаций";
	}

	public static String folderKey() {
		return "resultatyUtverjdenihSpecifikaciy";
	}
	...
	public String composeGetQuery(int queryKind) {
		...
		String p = "{" //
				+ "\"Дата\":\"" + Cfg.formatMills(dateFrom.value(), "yyyyMMdd") + "\""//
		...
		String serviceName = "РезультатыУтвержденныхСпецификаций";
		try {
	...
	public SubLayoutless getParametersView(Context context) {
		if (propertiesForm == null) {
			...
			propertiesForm//
					...
					.input(context, 1, Auxiliary.tapSize * 0.3, "Дата", new RedactDate(context).date.is(dateFrom).format.is("dd.MM.yyyy"))//
			...
			propertiesForm.child(new Knob(context)//
					...
					.afterTap.is(new Task() {
						...
						public void doTask() {
							//expectRequery.start(activityReports);
							activityReports.promptDeleteRepoort(ReportResultatyUtverjdenihSpecifikaciy.this, currentKey);
						}
					})//
					.left().is(propertiesForm.shiftX.property.plus(Auxiliary.tapSize * (0.3 + 0 * 2.5)))//
	...
	public String interceptActions(String s) {
		...
		for (int i = 0; i < strings.length; i++) {
			...
			if (i - 2 > -1 && i + 1 < strings.length - 1) {
				String numArt = extract(line, '№', '<');
				if (numArt.length() > 2) {
					String[] parts = numArt.split(".Арт");
					if (parts.length == 2) {
						String doc = parts[0];
						String art = parts[1];
