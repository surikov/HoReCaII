...
public class ReportDolgyPoNakladnim extends Report_Base {
	Numeric territory = new Numeric();
	Numeric dateFrom = new Numeric();
	Numeric dateTo = new Numeric();
	public static String menuLabel() {
		
		return "Долги по накладным";
	}
	public static String folderKey() {
		return "dolgyPoNakladnim";
	}
public  String getMenuLabel() {
	...
	public ReportDolgyPoNakladnim(ActivityWebServicesReports p) {
		super(p);
	}
	@Override
	...
	public SubLayoutless getParametersView(Context context) {
		if (propertiesForm == null) {
			...
			/*.input(context, 1, Auxiliary.tapSize * 0.3, "", new Knob(context).labelText.is("Обновить").afterTap.is(new Task() {
				@Override
				public void doTask() {
			...
			propertiesForm.child(new Knob(context)//
					...
					.afterTap.is(new Task() {
						...
						public void doTask() {
							//expectRequery.start(activityReports);
							activityReports.promptDeleteRepoort(ReportDolgyPoNakladnim.this, currentKey);
						}
					})//
							.left().is(propertiesForm.shiftX.property.plus(Auxiliary.tapSize * (0.3 + 0 * 2.5)))//
