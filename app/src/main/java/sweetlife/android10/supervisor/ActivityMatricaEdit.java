...
public class ActivityMatricaEdit extends Activity{
		...
		for(int i = 0; i < rows.size(); i++){
			...
			try{
				...
				sql = "insert into MatricaRowsX ("//
						...
						+ ",planTysRub,PlanSTM"//
						+ ",nacenka,uploaded,vrrab,email,deleteComment"//
						+ ")\n values\n ("//
						+ _id//
						+ "," + (row.child("Udalit").value.property.value().trim().equals("true") ? "1" : "false")
						...
						+ ",'" + row.child("Prichina").value.property.value() + "'"//
	...
	void fillMatricaGrid(){
		...
		for(int i = 0; i < currentMatrica.children.size(); i++){
			...
			try{
				...
				String deleteMark = "";
				if(row.child("Udalit").value.property.value().trim().equals("1")){
					deleteMark = "удалить: ";
				}
				mKontragent.cell(deleteMark + row.child("kontragent").value.property.value() + ": " + getKontragentName(row.child("kontragent").value.property.value())//
						, bg, tapMatricaRow);
	...
	void fillSvodGrid(){
		...
		for(int i = 0; i < currentSvod.children.size(); i++){
			Bough row = currentSvod.children.get(i);
			final String _id = row.child("_id").value.property.value();
			final String _utro = row.child("planUtro").value.property.value();
			final String _letuchka = row.child("planLetuchka").value.property.value();
			final String _stm = row.child("planSTM").value.property.value();
			final String data = row.child("data").value.property.value();
			Task tapSvodRow = new Task(){
				@Override
				public void doTask(){
					//tapSvodRow(_id);
					tapSvodRow(_id, _utro, _letuchka, _stm, data);
				}
			};
	...
	void matricaSend(final Note note, Task nextStep){//, String deleteCommentList){
		...
		String fileJson = "";
		String fileDelim = "";
		//json=json+(deleteCommentList);
		//if(currentMatrica.children.size() > 0 && deleteCommentList.trim().length() > 0){
		//	json=json+(",");
		//}
		for(int i = 0; i < currentMatrica.children.size(); i++){
			Bough row = currentMatrica.children.get(i);
			...
			//System.out.println("Kontragent "+i+": "+row.child("kontragent").value.property.value());
			if(i > 0){
				json = json + (",");
			}
			json = json + ("				{\"Kontragent\":" + row.child("kontragent").value.property.value() + ""
			...
			if(row.child("Udalit").value.property.value().trim().equals("1")){
				json = json + ("\n						,\"Udalit\":\"true\"");
				json = json + ("\n						,\"Prichina\":\"" + row.child("deleteComment").value.property.value() + "\"");
				String filePath = row.child("deletePath").value.property.value().trim();
				if(filePath.length() > 0){
					String rashirenie = filePath.substring(filePath.lastIndexOf(".") + 1);
					String base64 = android.util.Base64.encodeToString(
							sweetlife.android10.utils.SystemHelper.readBytesFromFile(
									new java.io.File(filePath)), android.util.Base64.NO_WRAP);

					fileJson = fileJson + fileDelim + "{"
		...
		for(int i = 0; i < currentSvod.children.size(); i++){
			...
			//System.out.println("Svod "+i+": "+data);
			if(i > 0){
				json = json + (",");
			}
			json = json + ("\n					{\"Data\":\"" + data + "\""
		...
		final String txt = json;
		System.out.println("matricaSend " + txt);
		//Auxiliary.writeTextToFile(new java.io.File("/sdcard/horeca/test.txt"), txt);
		//final Note dropLine = new Note();
		//final Note clientList = new Note();

		Expect expect = new Expect().status.is("Отправка")
				.task.is(new Task(){
					@Override
					public void doTask(){
						...
						String ok = raw.child("Сообщение").value.property.value();
						Vector<Bough> klients = raw.children("КонтрагентыСОшибками");
						note.value(note.value() + "" + ok + "\n");
						...
						if(ok.equals("ок")){
							...
							String delim = "";
							for(int ii = 0; ii < klients.size(); ii++){
								String dropKod = klients.get(ii).child("Kontragent").value.property.value();
								insertMarkClientDrop(dropKod);
								//clientList.value(clientList.value() + delim + klients.get(ii).child("KontragentImya").value.property.value());
								/*
								String oneJson="				{\"Kontragent\":" + klients.get(ii).child("Kontragent").value.property.value() + ""
										+ "\n						,\"Pn\":\"\""
										+ "\n						,\"Vt\":\"\""
										+ "\n						,\"Sr\":\"\""
										+ "\n						,\"Ct\":\"\""
										...
										+ "\n						,\"PtStrog\":\"false\""
										+ "\n						,\"SbStrog\":\"false\""
										+ "\n						,\"Prichina\":\"Удалено при редактировании " + Cfg.whoCheckListOwner() +    "\""
										+ "\n						,\"Plan\":\"0\""
										+ "\n						,\"PlanSTM\":\"0\""
										+ "\n						,\"VDM1\":\"0\""
										+ "\n						,\"VDM2\":\"0\""
										...
										+ "\n						,\"SbStrog1\":\"false\""
										+ "\n					}";
								dropLine.value(dropLine.value() + delim +oneJson);
										//"{\"Kontragent\": " + klients.get(ii).child("Kontragent").value.property.value()
										//+ ", \"Udalit\": true, "
										//+ "\"Prichina\": \"Удалено при редактировании " + Cfg.whoCheckListOwner() + "\""
										//+ "}");
								delim = ", ";
								*/
									...
									@Override
									public void doTask(){
										/*if(dropLine.value().length() > 1){
											promptDropClients(clientList.value(),  note,nextStep, dropLine.value());
										}else{
											nextStep.doTask();
										}*/
										nextStep.doTask();
	...
	void insertMarkClientDrop(String kod){
		String sql = "update MatricaRowsX set udalit=1 where matrica_id=" + _id + " and kontragent=" + kod + ";";
		ApplicationHoreca.getInstance().getDataBase().execSQL(sql);
		//int cnt = (int)Numeric.string2double(Auxiliary.fromCursor(ApplicationHoreca.getInstance().getDataBase().rawQuery("select changes() as cnt;", null)).child("row").child("cnt").value.property.value());
		int cnt = Auxiliary.lastUpdatedCount(ApplicationHoreca.getInstance().getDataBase());
		System.out.println("insertMarkClientDrop " + sql + "/" + cnt);
		if(cnt < 1){
			sql = "insert into MatricaRowsX ("//
					+ "matrica_id"
					+ ",udalit"
					...
					+ ",tipTT"
					+ ",tipOplaty"
					+ ",pn,vt,sr,ct,pt,sb,potencialTT,tom1,tom2,tom3,vdm1,vdm2,vdm3,planTysRub"//
					+ ",nacenka"//
					+ ",deleteComment"
					+ ")\n values\n ("//
					+ _id//
					...
					+ ",0"
					+ ",'Удалено " + Cfg.whoCheckListOwner() + "'"
					+ ");";
			ApplicationHoreca.getInstance().getDataBase().execSQL(sql);
			System.out.println("insertMarkClientDrop " + sql + "/" + Auxiliary.lastUpdatedCount(ApplicationHoreca.getInstance().getDataBase()));
		}
	}

	/*
		void promptDropClients(String clientList, final Note note,final Task nextStep, final String deleteComment){
			//System.out.println("promptDropClients " + deleteComment);
			Auxiliary.pick3Choice(this, "Удалить", clientList, "Удалить", new Task(){
						@Override
						public void doTask(){
							note.value("Удаление: ");
							matricaSend(note, nextStep);//, deleteComment);
						}
					}, "Не удалять", nextStep
					, null, null
			);
	...
	*/
	...
	void tapSvodRow(final String _id, String utro, String letuchka, String stm, String data){
		final Numeric planUtro = new Numeric().value(Numeric.string2double(utro));
		final Numeric planLetuchka = new Numeric().value(Numeric.string2double(letuchka));
		final Numeric planSTM = new Numeric().value(Numeric.string2double(stm));
		Date d = new Date();
		d.setTime((long)Numeric.string2double(data));
		SimpleDateFormat format = new SimpleDateFormat("dd.MM.yyyy, cccc");
		format.setTimeZone(TimeZone.getTimeZone("GMT+00:00"));
		String title = format.format(d);
		//Auxiliary.warn(""+_id+"/"+utro+"/"+letuchka+"/"+stm,this);
		Auxiliary.pick(this, title, new SubLayoutless(this)
						.field(this, 0, "план утро", new RedactNumber(this).number.is(planUtro))
						.field(this, 1, "план летучка", new RedactNumber(this).number.is(planLetuchka))
						.field(this, 2, "план СТМ", new RedactNumber(this).number.is(planSTM))
				...
				, "Сохранить", new Task(){
					...
					public void doTask(){
						...
						ApplicationHoreca.getInstance().getDataBase().execSQL(sql);
						requeryData();
						int nn = gridSvod.scrollView.getScrollY();
						fillSvodGrid();
						gridSvod.refresh();
						gridSvod.scrollView.scrollTo(0, nn);
					}
				}, null, null, null, null);
	}
}
