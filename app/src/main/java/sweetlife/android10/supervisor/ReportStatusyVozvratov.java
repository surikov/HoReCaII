...
public class ReportStatusyVozvratov extends Report_Base{
	...
	public void writeDefaultForm(String instanceKey){
		...
		b.child("docFrom").value.is("" + (d - 31 * 24 * 60 * 60 * 1000.0));
			...
			if(i - 2 > -1 && i + 1 < strings.length - 1){
				...
				if(num.length() > 2){//&& dat.trim().length() == 10 && ship.trim().length() == 10) {
					...
					if(i + 2 <= strings.length){
						String line2 = strings[i -1];
						int start2 = line2.indexOf('>');
						int end2 = line2.indexOf("<", start2 + 1);
						dat=line2.substring( start2+1,end2);
					}
