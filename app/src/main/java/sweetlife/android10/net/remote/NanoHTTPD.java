package sweetlife.android10.net.remote;
...
public class NanoHTTPD {
	...
	public Response serve(String uri, String method, Properties header, Properties parms, Properties files) {
		...
		while (e.hasMoreElements()) {
			...
			out("  HDR: '" + value + "' = '" + header.getProperty(value) + "'");
		}
		e = parms.propertyNames();
		while (e.hasMoreElements()) {
			String value = (String) e.nextElement();
			out("  PRM: '" + value + "' = '" + parms.getProperty(value) + "'");
		}
		e = files.propertyNames();
		while (e.hasMoreElements()) {
			String value = (String) e.nextElement();
			out("  UPLOADED: '" + value + "' = '" + files.getProperty(value) + "'");
		}
		...
		public void run() {
			try {
				...
				if (splitbyte < rlen) {
					...
					if (!sbfound || size == 0x7FFFFFFFFFFFFFFFl) {
						size = 0;
					}
				}
		...
		if (res == null && f.isDirectory()) {
			...
			if (res == null) {
				...
				if (new File(f, "index.html").exists()) {
					...
					if (new File(f, "index.htm").exists()) {
									...
									if (curFile.isFile()) {
										...
										if (len < 1024) {
											...
											if (len < 1024 * 1024) {
												...
												msg = msg + len / (1024 * 1024) + "." + len % (1024 * 1024) / 10 % 100 + " MB";
											}
										msg = msg + ")</font>";
									}
									msg = msg + "<br/>";
