...
public class BidsXMLParser extends DefaultHandler implements IParserBase {
	...
	public void startElement(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException {
		if (TAG_HEADER_DOC.compareToIgnoreCase(localName) == 0) {
			...
			if (TAG_RETURN.compareToIgnoreCase(localName) == 0) {
				mIsTrafiksOnly = true;
			}
			else
				if (TAG_STRING_TP.compareToIgnoreCase(localName) == 0) {
	...
	public void endElement(String namespaceURI, String localName, String qName) throws SAXException {
		if (TAG_HEADER_DOC.compareToIgnoreCase(localName) == 0) {
			...
			if (TAG_HELD.compareToIgnoreCase(localName) == 0) {
				mCurrentOrder.setHeld(mContentValue);
			}
			else
				if (TAG_ARTICLE.compareToIgnoreCase(localName) == 0) {
					mCurrentDeficit.article = mContentValue;
				}
				else
					if (TAG_DEFICIT.compareToIgnoreCase(localName) == 0) {
